...
class CMSTest(unittest.TestCase):
    ...
    def test_viewing_doc_content(self):
        self.create_document('history.txt', 'history')

        with self.client.get('/history.txt') as response:
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.content_type, 'text/plain; charset=utf-8')
            self.assertEqual(response.get_data(), b'history')
    ...
    def test_viewing_markdown_document(self):
        self.create_document('about.md', '<h1>Welcome to My CMS</h1>')
        response = self.client.get('/about.md')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, 'text/html; charset=utf-8')
        self.assertIn('<h1>Welcome to My CMS</h1>', response.get_data(as_text=True))
    ...
    def test_updating_document(self):
        ...
        client = self.admin_session()
        response = client.post('/changes.txt',
    ...
    def test_singin_form(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn('"username">Username:', response.get_data(as_text=True))
        self.assertIn('<button type="submit"', response.get_data(as_text=True))
